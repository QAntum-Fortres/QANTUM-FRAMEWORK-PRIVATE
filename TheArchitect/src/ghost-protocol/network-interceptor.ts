...
export class NetworkInterceptor {
    ...
    async startCapture(page: Page, testName: string): Promise<void> {
        ...
        logger.debug(`👻 [GHOST] Started capture for: ${testName}`);
    }
    ...
    private async handleRequest(request: Request, route: Route): Promise<void> {
        ...
        if (shouldCapture && this.session) {
            ...
            logger.debug(`👻 [GHOST] Captured: ${method} ${url}`);
        }
    ...
    private extractAuthToken(request: Request): void {
        ...
        if (auth && auth.startsWith('Bearer ')) {
            ...
            logger.debug(`👻 [GHOST] Extracted Bearer token`);
        }
    ...
    async stopCapture(): Promise<string> {
        ...
        logger.debug(`👻 [GHOST] Capture complete!`);
        logger.debug(`   📊 Requests captured: ${requestCount}`);
        logger.debug(`   ⏱️  UI test duration: ${duration}ms`);
        logger.debug(`   ⚡ Estimated API test: ~${Math.round(duration / 50)}ms (${Math.round(duration / (duration / 50))}x faster)`);
...
╚══════════════════════════════════════════════════════════════╝
`);
    logger.debug(`📂 Input: ${uiTestPath}`);
    logger.debug(`⏳ Running UI test with network capture...`);
    logger.debug('');
