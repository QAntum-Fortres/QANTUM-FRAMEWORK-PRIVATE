...
impl MetaLogicEngine {
    ...
    fn initialize_systems(&mut self) {
        ...
        let c2 = self.create_proposition("LAW_NONCONTRADICTION", "!(A & !A)", TruthValue::TRUE, None);
        let c3 = self.create_proposition("LAW_EXCLUDED_MIDDLE", "A | !A", TruthValue::TRUE, None);

        self.systems.insert("CLASSICAL".to_string(), LogicalSystem {
        ...
        let p2 = self.create_proposition("PARA_TOLERANCE", "A & !A can exist without explosion", TruthValue::TRUE, None);

        self.systems.insert("PARACONSISTENT".to_string(), LogicalSystem {
        ...
        // Catuskoti
        let cat1 = self.create_proposition("CATU_1", "Position 1: A is true", TruthValue::TRUE, Some(1));
        let cat2 = self.create_proposition("CATU_2", "Position 2: A is false", TruthValue::FALSE, Some(2));
        let cat3 = self.create_proposition("CATU_3", "Position 3: A is both true and false", TruthValue::BOTH, Some(3));
        let cat4 = self.create_proposition("CATU_4", "Position 4: A is neither true nor false", TruthValue::NEITHER, Some(4));

        self.systems.insert("CATUSKOTI".to_string(), LogicalSystem {
        ...
        // Meta System
        let m1 = self.create_proposition("META_GODEL", "Any consistent system powerful enough to express arithmetic is incomplete", TruthValue::TRUE, None);
        let g_sent = self.create_proposition("G", "This statement is not provable in system S", TruthValue::UNDEFINED, None);

        self.systems.insert("META".to_string(), LogicalSystem {
         ...
         // Pataphysics
        let pata1 = self.create_proposition("PATA_IMAGINE", "Imaginary solutions are as valid as real ones", TruthValue::IMAGINARY, None);

        self.systems.insert("PATAPHYSICS".to_string(), LogicalSystem {
    ...
    fn create_proposition(&mut self, id: &str, content: &str, truth_value: TruthValue, catuskoti_position: Option<usize>) -> MetaProposition {
        let lower_content = content.to_lowercase();
        let self_ref = lower_content.contains("this") && (lower_content.contains("statement") || lower_content.contains("sentence"));
    ...
    pub fn query(&mut self, question: &str) -> QueryResult {
        ...
        let mut current_answer = TruthValue::UNDEFINED;
        ...
        if classical_result.truth_value != TruthValue::UNDEFINED && classical_result.truth_value != TruthValue::PARADOX {
            current_answer = classical_result.truth_value;
            reasoning.push(format!("   Classical answer: {:?}", current_answer));
        } else {
            reasoning.push("   Classical logic insufficient - contains paradox or undecidable".to_string());
            ...
            reasoning.push(format!("   Catuṣkoṭi position: {:?} ({:?})", catuskoti_result.catuskoti_position, catuskoti_result.truth_value));
            ...
            if self.is_godelian_limit(question) {
                ...
                reasoning.push("   Imaginary solution generated - valid in pataphysical space".to_string());
            } else {
        ...
        QueryResult {
            ...
            reasoning,
            systems_consulted,
            godel_limit: if golden_key_used { Some("Gödel incompleteness encountered - transcended via pataphysics".to_string()) } else { None },
            transcendence_method: if golden_key_used { Some("PATAPHYSICS + GOLDEN_KEY".to_string()) } else { Some("CATUSKOTI".to_string()) },
            golden_key_used,
        }
    }
    ...
    fn classical_evaluate(&mut self, question: &str) -> MetaProposition {
        if self.is_paradox(question) {
            return self.create_proposition(&format!("EVAL_{}", chrono::Utc::now().timestamp_millis()), question, TruthValue::PARADOX, None);
        }

        // Simple logic for example purposes
        let lower_q = question.to_lowercase();
        if lower_q.contains("not") && lower_q.contains("is") {
             return self.create_proposition(&format!("EVAL_{}", chrono::Utc::now().timestamp_millis()), question, TruthValue::UNDEFINED, None);
        }

        self.create_proposition(&format!("EVAL_{}", chrono::Utc::now().timestamp_millis()), question, TruthValue::TRUE, None)
    }
    ...
    fn catuskoti_evaluate(&mut self, question: &str) -> MetaProposition {
        ...
        self.create_proposition(&format!("CATU_EVAL_{}", chrono::Utc::now().timestamp_millis()), question, truth_value, Some(position))
    }
    ...
    fn dialectic_synthesize(&mut self, question: &str, current_answer: TruthValue) -> DialecticTriad {
        ...
        let synthesis = MetaProposition {
            ...
            content: format!("The answer transcends {:?} and its negation", current_answer),
