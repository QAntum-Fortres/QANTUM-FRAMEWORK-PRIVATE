...
import { Eye, Play, ShieldCheck, Activity, BrainCircuit, Scan, Cpu, Layers, Zap, Loader2, Globe } from "lucide-react";
import { veritas } from "@/veritas_sdk/VeritasClient";
import { AgentStep, VisionResult, ObserverState, SwarmStatus } from "@/veritas_sdk/types";
...
export function VeritasControlCenter() {
    ...
    const [status, setStatus] = useState<"IDLE" | "SCANNING" | "EXECUTING" | "COMPLETE" | "SWARMING">("IDLE");
    ...
    const handleAnalyze = async () => {
        ...
        // Also trigger observer check
        const obs = await veritas.observe("http://localhost:3000");
    ...
    const handleSwarm = async () => {
        setStatus("SWARMING");
        setSwarmStatus(null);
        const result = await veritas.launchSwarm(goal, 1000);
                ...
                <CardHeader className="border-b border-border/10 pb-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            {observerState && (
                                <Badge variant="outline" className={`${observerState.stable ? 'text-green-400 border-green-500/30' : 'text-red-400 border-red-500/30'}`}>
                                    AMNIOTIC STATE: {(observerState.amniotic_state_score * 100).toFixed(0)}%
                                </Badge>
                            )}
                            <Badge role="status" aria-live="polite" variant="outline" className={`
                                ${status === 'EXECUTING' || status === 'SWARMING' ? 'bg-amber-500/10 text-amber-400 border-amber-500/30 animate-pulse' : ''}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                            ...
                            <div className="aspect-video bg-slate-900/50 rounded-lg border border-border/20 flex items-center justify-center relative overflow-hidden group">
                                ...
                                {/* Heatmap Overlay */}
                                {visionResult?.heatmap_data && (
                                    <div className="absolute inset-0 grid grid-cols-10 opacity-30 pointer-events-none">
                                        {visionResult.heatmap_data.map((val, i) => (
                                            <div key={i} style={{ backgroundColor: `rgba(255, 0, 0, ${val})` }} />
                                        ))}
                                    </div>
                                )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button onClick={handleSwarm} disabled={status !== 'IDLE'} variant="destructive" className="bg-purple-600 hover:bg-purple-700">
                                        <Globe className="h-4 w-4 mr-2" /> SWARM
                                    </Button>
                                </div>
                            </div>

                            <div className="bg-slate-950 rounded-md border border-border/20 p-4 h-[200px] overflow-y-auto font-mono text-xs">
                                {status === 'SWARMING' || swarmStatus ? (
                                    <div className="space-y-3">
                                        <div className="text-purple-400 font-bold border-b border-purple-500/20 pb-2 mb-2 flex items-center gap-2">
                                            <Globe className="h-4 w-4" /> DISTRIBUTED SWARM ACTIVE
                                        </div>
                                        {status === 'SWARMING' && (
                                            <div className="flex items-center gap-2 text-slate-300 animate-pulse">
                                                <Loader2 className="h-3 w-3 animate-spin" /> Provisioning 1000 Micro-Agents...
                                            </div>
                                        )}
                                        {swarmStatus && (
                                            <>
                                                <div className="grid grid-cols-2 gap-2 mb-4">
                                                    <div className="bg-purple-900/20 p-2 rounded">
                                                        <div className="text-slate-500">Active Agents</div>
                                                        <div className="text-xl text-purple-300">{swarmStatus.active_agents}</div>
                                                    </div>
                                                    <div className="bg-purple-900/20 p-2 rounded">
                                                        <div className="text-slate-500">Throughput</div>
                                                        <div className="text-xl text-purple-300">{swarmStatus.throughput_tps.toFixed(0)} TPS</div>
                                                    </div>
                                                </div>
                                                {swarmStatus.logs.map((log, i) => (
                                                    <div key={i} className="text-slate-400 border-l-2 border-purple-500/30 pl-2">
                                                        {log}
                                                    </div>
                                                ))}
                                            </>
