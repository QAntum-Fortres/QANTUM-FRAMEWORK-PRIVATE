...
async fn run_mojo_scan(State(state): State<Arc<ServerState>>) -> impl IntoResponse {
    match crate::neuro::mojo_bridge::MojoBridge::scan_void_optimized(state.vsh.clone()).await {
        Ok(report) => {
            Json(json!({ "status": "SUCCESS", "report": report, "engine": "MOJO_HYBRID" }))
        }
...
pub async fn start_singularity_server(state: Arc<ServerState>) {
    ...
    let app = Router::new()
        ...
        .route("/api/mojo/scan", post(run_mojo_scan))
        .with_state(state)
