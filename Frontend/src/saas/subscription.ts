...
export class SubscriptionService {
   ...
   * Record usage
   */
  recordUsage(subscriptionId: string, metric: keyof Usage['metrics'], amount: number = 1): void {
  ...
  private updatePercentages(usage: Usage): void {
    ...
    usage.percentages = {
      ...
      storage:
        limits.storageGB === -1
          ? 0
          : (metrics.storageBytes / (limits.storageGB * 1024 * 1024 * 1024)) * 100,
  ...
  private getMetricValue(usage: Usage, metric: keyof PlanLimits): number {
    switch (metric) {
      case 'projects':
        return usage.metrics.projects;
      case 'users':
        return usage.metrics.users;
      case 'testsPerMonth':
        return usage.metrics.tests;
      case 'storageGB':
        return usage.metrics.storageBytes / (1024 * 1024 * 1024);
      case 'apiCallsPerDay':
        return usage.metrics.apiCalls;
      case 'retentionDays':
        return 0;
      default:
        return 0;
